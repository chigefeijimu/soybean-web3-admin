<template>
  <div class="identity-reputation">
    ...
    <div class="stats-grid" v-if="platformStats">
      ...
      <div class="stat-card">
        <div class="stat-value">{{ platformStats.averageReputation.toFixed(1) }}</div>
    ...
    <div class="main-content">
      ...
      <div class="panel">
        ...
        <div class="form-group">
          ...
          <div class="social-inputs">
            ...
            <input v-model="socialLinks.website" placeholder="Website URL" class="input" />
          </div>
        </div>
      ...
      <div class="panel">
        ...
        <div v-if="currentProfile" class="profile-card">
          ...
          <div class="bio-section" v-if="currentProfile.bio">
            <h4>About</h4>
            <p>{{ currentProfile.bio }}</p>
          </div>

          <div class="social-section" v-if="currentProfile.socialLinks">
            <h4>Links</h4>
            <div class="social-links">
              <a v-if="currentProfile.socialLinks.twitter" :href="`https://twitter.com/${currentProfile.socialLinks.twitter}`" target="_blank">
                𝕏 {{ currentProfile.socialLinks.twitter }}
              </a>
              <a v-if="currentProfile.socialLinks.github" :href="`https://github.com/${currentProfile.socialLinks.github}`" target="_blank">
                🐙 {{ currentProfile.socialLinks.github }}
              </a>
              <a v-if="currentProfile.socialLinks.website" :href="currentProfile.socialLinks.website" target="_blank">
    ...
    <div class="panel cross-chain" v-if="currentProfile">
      ...
      <div class="chain-inputs">
        ...
        <input v-model="txCount" type="number" placeholder="Transaction count" class="input" />
        <button @click="addChainIdentity" class="btn btn-secondary">Add Chain</button>
      </div>
...
.badges-section, .bio-section, .social-section {
  margin-bottom: 1rem;
}

.badges-section h4, .bio-section h4, .social-section h4 {
  margin-bottom: 0.5rem;
  font-size: 0.9rem;
}
...
.cross-chain .chain-inputs {
  display: flex;
  gap: 0.5rem;
  margin-bottom: 1rem;
}

.cross-chain .select, .cross-chain .input {
  flex: 1;
}
