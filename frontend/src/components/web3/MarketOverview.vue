...
import { getTopCoins, getRealPrice } from '@/service/api/web3'
...
<template>
  <div class="bg-slate-800/50 backdrop-blur-xl rounded-2xl border border-slate-700/50 overflow-hidden">
    ...
    <div v-if="loading" class="p-8 text-center">
      ...
      <p class="text-slate-400 mt-2">Loading market data...</p>
    </div>
    ...
    <div v-else class="divide-y divide-slate-700/50 max-h-[600px] overflow-y-auto">
      ...
      >
        <div class="flex items-center gap-3">
          ...
          <!-- Icon -->
          <div class="w-10 h-10 rounded-full bg-gradient-to-br from-purple-500 to-blue-500 flex items-center justify-center font-bold">
          ...
          <div class="flex-1 min-w-0">
            <div class="flex items-center gap-2">
              ...
              <span class="text-slate-500 text-sm truncate">{{ token.name }}</span>
            </div>
            <div class="text-xs text-slate-500">
              Vol: {{ formatVolume(token.volume24h) }} • MCap: {{ formatMarketCap(token.marketCap) }}
            </div>
          </div>
          ...
          <div class="text-right">
            ...
            <p :class="['text-sm font-medium', token.change24h >= 0 ? 'text-green-400' : 'text-red-400']">
