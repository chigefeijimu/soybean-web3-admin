...
<template>
  <div class="bg-slate-800/50 backdrop-blur-xl rounded-2xl border border-slate-700/50 overflow-hidden">
    ...
    <!-- Column Headers -->
    <div class="grid grid-cols-3 gap-2 px-4 py-2 text-xs text-slate-500 border-b border-slate-700/50">
    ...
    <div class="divide-y divide-slate-700/30">
      <div 
        ...
        :key="'sell-' + i"
        class="grid grid-cols-3 gap-2 px-4 py-1.5 text-sm relative"
      >
        <div 
          class="absolute inset-y-0 right-0 bg-red-500/10"
          :style="{ width: getDepthWidth(order.total) + '%' }"
        />
        <span class="text-red-400 relative z-10">{{ formatPrice(order.price) }}</span>
        <span class="text-right relative z-10">{{ formatAmount(order.amount) }}</span>
        <span class="text-right text-slate-400 relative z-10">{{ formatTotal(order.total) }}</span>
      </div>
    </div>

    <!-- Mid Price -->
    <div class="py-3 px-4 bg-slate-700/30 border-y border-slate-700/50">
      <div class="text-center">
        <span class="text-xl font-bold text-green-400">
          ${{ (sellOrders[0].price + buyOrders[0].price) / 2 }}
        </span>
        <span class="text-xs text-slate-500 ml-2">Mid Price</span>
      </div>
    </div>
    ...
    <div class="divide-y divide-slate-700/30">
      ...
      >
        <div 
          ...
          :style="{ width: getDepthWidth(order.total) + '%' }"
        />
        <span class="text-green-400 relative z-10">{{ formatPrice(order.price) }}</span>
        <span class="text-right relative z-10">{{ formatAmount(order.amount) }}</span>
        <span class="text-right text-slate-400 relative z-10">{{ formatTotal(order.total) }}</span>
      </div>
    </div>
    ...
    <!-- Footer Stats -->
    <div class="p-3 border-t border-slate-700/50 grid grid-cols-2 gap-2 text-xs">
      <div class="text-center">
