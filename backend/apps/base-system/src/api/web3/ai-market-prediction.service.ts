...
export class AiMarketPredictionService {
   ...
   */
  async getPredictionHistory(symbol: string, days: number = 7): Promise<{date: string; prediction: string; confidence: number; price: number}[]> {
    const history: {date: string; prediction: string; confidence: number; price: number}[] = [];
    const now = new Date();
