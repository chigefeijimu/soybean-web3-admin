...
@Injectable()
export class CrossChainPriceApiService {
  ...
  async getAllCrossChainPrices(): Promise<CrossChainPrice[]> {
    ...
    return results.filter((r): r is CrossChainPrice => r !== null);
  }
  ...
  async getPriceComparison(symbol: string) {
    ...
    const arbitrageOpportunity = potentialProfit / lowestPrice.price > 0.02;
