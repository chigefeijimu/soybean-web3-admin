...
export class OptionsTrackerService {
  ...
  async getOpenInterest(asset: string, timeframe: string) {
    ...
    const history: Array<{timestamp: string; callOi: number; putOi: number; totalOi: number}> = [];
  ...
  async getPutCallRatio(asset: string, expiry: string) {
    ...
    const history: Array<{date: string; ratio: number; callVolume: number; putVolume: number}> = [];
    for (let i = 30; i >= 0; i--) {
      const date = new Date();
  ...
  async getOptionsVolume(asset: string, period: string) {
    ...
    const history: Array<{date: string; callVolume: number; putVolume: number; totalVolume: number}> = [];
  ...
  private generateStrikes(opt: OptionsData, targetStrike: string): Array<{strike: number; callIv: number; putIv: number; delta: number}> {
    const basePrice = opt.maxPain;
    const strikes: Array<{strike: number; callIv: number; putIv: number; delta: number}> = [];
