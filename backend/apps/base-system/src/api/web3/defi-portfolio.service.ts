...
const TOKEN_ADDRESSES: Record<number, Record<string, string>> = {
  ...
  42161: {
    ...
    WETH: '0x82aF49447D8a07e3bd95BD0d56f78341539c28bE',
  },
};
...
export class DefiPortfolioService {
  ...
  async getPortfolio(walletAddress: string, chainId: number = 1, protocols?: string): Promise<DefiPortfolio> {
    ...
    const weightedApy = totalValueUsd > 0 
      ? filteredPositions.reduce((sum, p) => sum + (p.apy * p.valueUsd), 0) / totalValueUsd
      : 0;
  ...
  private async getDefiPositions(
    ...
    if (tokenBalances['ETH'] || tokenBalances['WETH']) {
      positions.push({
        protocol: 'uniswap',
        symbol: 'ETH-USDC',
        tokenAddress: '0x8ad599c3A0ff1De082011EFDDc58f1908eb6e6D8',
  ...
  private generateProtocolBreakdown(positions: DefiPosition[]): Record<string, number> {
