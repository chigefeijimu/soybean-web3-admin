...
async function getPortfolio(address: string, chainId: number): Promise<Portfolio> {
  try {
    if (chainId === 1 || chainId === 5) {
      ...
      const tokens: TokenHolding[] = response.data?.tokens 
...
function getDemoPortfolio(address: string): Portfolio {
  const hash = address.split('').reduce((a, b) => a + b.charCodeAt(0), 0);
  const seed = hash % 1000;
  
  const demoTokens: TokenHolding[] = [
...
app.get('/portfolio-comparator/compare', async (req, res) => {
  ...
  try {
    const [portfolioA, portfolioB] = await Promise.all([
      getPortfolio(addressA as string, parseInt(chainId as string)),
      getPortfolio(addressB as string, parseInt(chainId as string)),
    ]);
    ...
    const commonTokens: TokenHolding[] = [];
    const uniqueToA: TokenHolding[] = [];
    const uniqueToB: TokenHolding[] = [];
...
app.get('/portfolio-comparator/rank', async (req, res) => {
  ...
  const addrList = (addresses as string).split(',').filter(Boolean);
  
  try {
    const portfolios = await Promise.all(
      addrList.map(async (addr) => {
        const portfolio = await getPortfolio(addr, parseInt(chainId as string));
