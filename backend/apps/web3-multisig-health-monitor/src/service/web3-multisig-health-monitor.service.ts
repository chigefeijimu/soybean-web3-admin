...
export interface AlertConfig {
  ...
  threshold: number;
  type: 'pending_tx' | 'inactivity' | 'signer_change' | 'low_quorum';
  enabled: boolean;
  webhookUrl?: string;
  email?: string;
}
...
@Injectable()
export class Web3MultisigHealthMonitorService {
  ...
  private readonly commonMultisigABIs = [
  ...
  async getHealthTrends(walletAddress: string, chainId: number, days = 30) {
    // Generate simulated historical health data
    const trends = [];
    const now = Date.now();
  ...
  private async fetchMultisigData(walletAddress: string, chainId: number, chainConfig: any) {
    // Simulate fetching multi-sig data from blockchain
  ...
  private async analyzeSignerActivity(owners: string[], chainId: number, chainConfig: any): Promise<SignerActivity[]> {
    // Simulate signer activity analysis
  ...
  private calculateHealthMetrics(walletData: any, signerActivity: SignerActivity[]): HealthMetrics {
    ...
    const activeSigners = signerActivity.filter(s => s.activityScore > 20).length;
    
    let score = 70; // Base score
    
    // Adjust based on threshold vs owner count
    const ratio = walletData.threshold / walletData.owners.length;
    if (ratio >= 0.5) score += 10; // Good threshold balance
    else if (ratio >= 0.33) score += 5;
    
    // Adjust based on signer activity
    if (avgActivity > 50) score += 10;
    else if (avgActivity > 30) score += 5;
    
    // Adjust based on owner count
