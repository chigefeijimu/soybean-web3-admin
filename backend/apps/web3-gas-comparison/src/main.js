...
function getDemoGasData(chainId) {
  const config = CHAIN_CONFIG[chainId];
  const ethPrice = 2450;
...
async function compareGasPrices() {
  ...
  const chains = (await Promise.all(gasDataPromises)).filter(d => d !== null);
...
// Routes
app.get('/gas-comparison', async (req, res) => {
  try {
...
app.get('/gas-comparison/chains', (req, res) => {
...
app.get('/gas-comparison/:chainId', async (req, res) => {
