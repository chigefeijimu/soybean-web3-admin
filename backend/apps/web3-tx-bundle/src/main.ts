...
@Module({
  controllers: [TxBundleController],
})
class TxBundleAppModule {}

async function bootstrap() {
  const app = await NestFactory.create(TxBundleAppModule);
  app.enableCors();
  await app.listen(3002);
  console.log('🚀 Transaction Bundle API running on http://localhost:3002');
}
bootstrap();
